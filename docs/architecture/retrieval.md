...
- **`/health?deep=1` answers it before a run, not after one.** The roster above is decided **once, at boot**, from the config's backend credentials — so it is knowable the moment the app is up, and the run that discovers it the hard way spends 38 minutes to reach INSUFFICIENT DATA. The endpoint returns `sources_declared` (a count, since *0 unavailable* and *18 of 30* are different answers and only the second names a credential to go and set) and `sources_unavailable` (`{source: reason}`), beside the `storage_ok` that already existed for the same reason one stage later. The UI dot collects **all four** amber states — no LLM credential, an unusable store, unqueryable sources, and a knowledge pack that loaded empty — rather than ranking them, because a deployment missing one credential usually misses several and reporting only the first makes each fix reveal the next. Each hover names the *consequence*: `401`, "lost on the next restart", "INSUFFICIENT DATA — not an empty result", "every condition `unknown`". None turns the dot red: the server **is** up in all four, which is exactly what made them invisible, and conflating them with unreachable is how an indicator starts flapping.

The fourth is the widest and it arrived last, because it is the one whose *symptom* is a healthy-looking config: `pack` used to report whether `knowledge.pack_dir` was a non-empty string. `load_knowledge_pack` answers a directory it cannot read with an **empty** `KnowledgePack` and one `logger.warning`, so a pack that never shipped — or a `pack_dir` still holding the template's `<pack-directory>` — reported `true`, then cost the entity glossary, the source catalog and every ruleset at once. It is now counted from the loaded pack's own contents (`_pack_health`, `src/incident_input.py`), with `pack_detail` carrying the name and the four counts **whether or not it loaded**: a pack that loaded still has to be read for the ruleset a verdict needs, and `0 rulesets` is the only thing that says so. Entities and sources are the essentials, and a ruleset is deliberately not — a pack may legitimately ship none and still retrieve and correlate.
...
- **It answers a reasoning defect by bypassing the reasoning.** A declared source the planner declined is a defect in what the catalog *tells* the planner — a missing `selection_guidance`, a `description` that reads like an inventory entry. Fixed there, every future incident benefits; injected behind the planner, one incident is patched and the cause is hidden. The measurement that made this actionable replays each persisted understanding, runs *only* query generation, and reports `PICKED` / `DECLARED` / `UNSCOPABLE` / `MISSING`, where a non-empty `MISSING` is the selection defect to repair in the pack.
...
**`window: onwards` makes an incident's retrieval cost grow with the incident's AGE**, which is
correct for the question it asks — *did the receivers go on to read the records?* has no end date —
and is also why a stored incident replayed months later is not comparable to its original run.
`_follow_up_window` resolves `onwards` to `event start → today` (`api_call_generator.py:756`), so the
scan widens by a day per day. Measured on one incident text run twice against the same deployment:
on 2026-08-10 the pass-2 target was asked over **5 days** and answered in **478s with 0 rows** — an
*empty* source, discounted as expected; on 2026-09-08 the same target, with byte-identical harvested
values, was asked over **34 days** (6.8×) and **timed out at its 1200s budget** (478s / 5 days ≈ 96
s/day projects to ~3250s, which is the whole of it — this was not warehouse load). The verdict held
either way, and the two outcomes read *differently* exactly as they must: `0 rows` landed under
"EMPTY IS THE ANSWER, not a gap", while the timeout landed under `SOURCE DID NOT ANSWER — … its rows
are MISSING, not empty, so every condition below that reads it is unresolved rather than answered`.
So the failure is visible; the trap is comparing the two runs and reading a pack or code change into
what is a calendar. An author who wants a bounded sweep declares `lookback`/`inherit` or raises that
source's budget; the engine holds no depth of its own here for the same reason it holds none for
`default_lookup_days`.
