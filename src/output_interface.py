...
class OutputInterface:
    ...
    async def send_email(self, report, incident_id):
        try:
            ...
            msg.attach(
                MIMEText(
                    f"Please find attached the fraud investigation report for incident {incident_id}."
                )
            )
            ...
            async with aiosmtplib.SMTP(
                hostname=self.config["smtp_server"], port=self.config["smtp_port"]
            ) as smtp:
                await smtp.starttls()
                await smtp.login(
            ...
            logger.info(
                f"Sent fraud investigation report for incident {incident_id} via email"
            )
    ...
    async def save_to_file(self, report, incident_id):
        try:
            ...
            # report is PDF bytes or a text/JSON string depending on
            # report_generation.output_format, so mode and extension follow its type.
            is_bytes = isinstance(report, (bytes, bytearray))
            ext = "pdf" if is_bytes else "txt"
            filename = f"fraud_report_{incident_id}.{ext}"
            filepath = os.path.join(output_dir, filename)

            async with aiofiles.open(filepath, "wb" if is_bytes else "w") as f:
                await f.write(report)

            logger.info(
                f"Saved fraud investigation report for incident {incident_id} to {filepath}"
            )
        except Exception as e:
            logger.error(
                f"Error saving report to file for incident {incident_id}: {str(e)}"
            )
            raise
    ...
    async def send_to_api(self, report, incident_id):
        try:
            ...
            # aiohttp has no `files=` kwarg (that is requests), so the multipart body
            # is built with FormData.
            form = aiohttp.FormData()
            form.add_field("incident_id", str(incident_id))
            form.add_field(
                "report",
                report,
                filename=f"fraud_report_{incident_id}.pdf",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Successfully sent fraud investigation report for incident {incident_id} to API"
                        )
                    else:
                        raise Exception(f"API returned status code {response.status}")
        except Exception as e:
            logger.error(
                f"Error sending report to API for incident {incident_id}: {str(e)}"
            )
            raise
...
async def main():
    ...
    # Stand-in for the generated PDF bytes.
